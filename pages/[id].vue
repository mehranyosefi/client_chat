<template>
   <section id="chat-room"
      ...
      <Suspense>
         <template #default>
            <CahtRoom :user_id="(route.params.id as string)" />
         </template>
         <template #fallback>
...
definePageMeta({
  ...
  middleware (to, from) {
   ...
   to.meta.pageTransition!.name = innerWidth.value > 768 ? 'fade' : to.params.id ?  'slide-right' : 'slide-left'
   to.meta.pageTransition!.mode = innerWidth.value > 768 ? 'out-in' : to.params.id ?  'out-in' : 'out-in'
  }
})
